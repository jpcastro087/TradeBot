		...
		while (true) {
			...
			while (true) {
				try {
					...
					Mode.set(Mode.valueOf("live".toUpperCase()));
					break;
				} catch (Exception e) {
			...
			if (Mode.get() == Mode.COLLECTION) {
				...
				System.out.println("---" + (Mode.get().equals(Mode.BACKTESTING) ? "Backtesting" : "Setup")
						+ " finished (" + Formatter.formatDecimal(time) + " s)\n");
				...
				while (!returnToModes) {
					...
					case "wallet":
						System.out.println("\nTotal wallet value: "
								+ Formatter.formatDecimal(localAccount.getTotalValue()) + " " + ConfigSetup.getFiat());
						System.out
								.println(Formatter.formatDecimal(localAccount.getFiat()) + " " + ConfigSetup.getFiat());
						for (Map.Entry<Currency, Double> entry : localAccount.getWallet().entrySet()) {
							if (entry.getValue() != 0) {
								System.out.println(Formatter.formatDecimal(entry.getValue()) + " "
										+ entry.getKey().getPair().replace(ConfigSetup.getFiat(), "") + " ("
										+ Formatter.formatDecimal(entry.getKey().getPrice() * entry.getValue()) + " "
										+ ConfigSetup.getFiat() + ")");
							}
						}
						break;
